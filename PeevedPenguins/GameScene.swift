...
class GameScene: SKScene, SKPhysicsContactDelegate {
    ...
    func setupCatapult() {
        ...
        catapultSpringJoint.frequency = 3
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if nodeAtPoint.name == "catapultArm" {  // If the touched node is named "catapultArm" do...
            ...
            let penguin = Penguin()
            addChild(penguin)
            penguin.position.x += catapultArm.position.x + 40 // ***
    ...
    override func update(_ currentTime: CFTimeInterval) {
        ...
        if let cameraTarget = cameraTarget {
            ...
            if (cameraTarget.physicsBody!.joints.count == 0 &&
                cameraTarget.physicsBody!.velocity.length() < 0.18) ||
                (cameraTarget.position.x > 700 ||
                    cameraTarget.position.x < -300 ||
